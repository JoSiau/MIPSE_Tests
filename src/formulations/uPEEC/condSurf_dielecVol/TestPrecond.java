...
public class TestPrecond {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Logger.getLogger(TestPrecond.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        /*
         cas=1;
         ImportFlux mesh = new ImportFlux(meshDir + "/magnetodynamic/U_PEEC_DIELECTRIC/BIG_BIG_SNAKE_TETRA.DEC");
         /*/
        cas = 2;
        ImportFlux mesh = new ImportFlux(meshDir + "/magnetodynamic/U_PEEC_DIELECTRIC/BIG_SNAKE_TETRA.DEC");
        //*/
        double sigma = 1 / 1.72E-8;
        double epsilon = 1.0 * eps0;
        double ep = 35e-6;
        ...
        if (cas == 1) {
            circuitPur.addSourceISimple(solP.getNbLignes(), 15033, 15037, "Commun Borne1", 1.0, 0);
        } else if (cas == 2) {
            circuitPur.addSourceISimple(solP.getNbLignes(), 9882, 9884, "Source", 1.0, 0.0);
        }
        int nbBranches = solP.getNbLignes() + 1;
//
...
//        solP.setExportTopologie("D:");
        // Resolution
        //*
        solP.setCompression(true);// Enleve la compression
        solP.setParamIterativeSolver(1, 1e6, -1e-8, 1, -1);
        double ib[][] = solP.resolutionIterativePrecond(f);
        /*/
