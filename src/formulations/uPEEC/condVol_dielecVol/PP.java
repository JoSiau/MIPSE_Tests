...
//        String file = path + "/src/g2elab/mipse/formulationInProgress/magnetodynamic/LOOPANTENNA_SMALL.DEC";
        String file = path + "/src/g2elab/mipse/formulationInProgress/magnetodynamic/U_PEEC_DIELECTRIC/2PP_VOL.DEC";
        ...
        solP.setPtsDeGaussInductifs(8, 8, 27, true);
...
//        double ib[][] = solP.resolutionDirecte(f);
        ...
        try {
            // Save the datas.
            Ecriture saveRes = new Ecriture("D:/jsiau/_Backup_Sources/Resultats/Dielectric/2PP/"+ Omega.getActiveDofCount() + "f" + f + "_RES.out");
            for (int i = 0; i < 2; i++) {
        ...
        ExportGmshHdiv exportJreal = new ExportGmshHdiv(Omega, "D:/jsiau/_Backup_Sources/Resultats/Dielectric/2PP/" + Omega.getActiveDofCount() + "f" + f + "_RE.msh");
        exportJreal.addQuantity(Jreal, "Jreal");
//        exportJreal.vizualiseFaceValue(Jreal);

        ExportGmshHdiv exportJimag = new ExportGmshHdiv(Omega, "D:/jsiau/_Backup_Sources/Resultats/Dielectric/2PP/" + Omega.getActiveDofCount() + "f" + f + "_IM.msh");
...
//        exportJimag.vizualiseFaceValue(Jimag);
        ...
        exportJreal = new ExportGmshHdiv(Omega, "D:/jsiau/_Backup_Sources/Resultats/Dielectric/2PP/" + Omega.getActiveDofCount() + "f" + f + "_MOD.msh");
        exportJreal.addQuantityExportMod(J, "Jmod");

//        ExportVtkHdiv tmp = new ExportVtkHdiv(Omega, "D:/jsiau/_Backup_Sources/Resultats/Dielectric/2PP/Jmod.vtk");
//        tmp.addQuantity(J, "J");

        File out_file = new File("D:/jsiau/_Backup_Sources/Resultats/Dielectric/2PP/" + Omega.getActiveDofCount() + "f" + f + "_RE.msh");
        File out_file1 = new File("D:/jsiau/_Backup_Sources/Resultats/Dielectric/2PP/" + Omega.getActiveDofCount() + "f" + f + "_IM.msh");
        File out_file2 = new File("D:/jsiau/_Backup_Sources/Resultats/Dielectric/2PP/" + Omega.getActiveDofCount() + "f" + f + "_MOD.msh");
        try {
