...
import g2elab.mipse.formulationInProgress.magnetodynamic.U_PEEC_DIELECTRIC.Cond_n_Dielec_Volumic.solverPEEC_CondInDielec;
 ...
 * @author jsiau
 */
public class PEEC_RLMPC_SNAKE_CID {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // Create the solver
        solverPEEC_CondInDielec solP = new solverPEEC_CondInDielec(rV[0], 1 / 68e-8, new VolumeRegion[]{rV[1]}, new double[]{4.7 * eps0}, (SurfaceRegion) Imp.getRegion(2), (SurfaceRegion) Imp.getRegion(3), true);
        solP.setPtsDeGaussInductifs(8, 8, 27, false);
        solP.setPtsDeGaussCapacitifs(25, 25, true);
...
//        exg = new ExportGmsh(esh, (defaultPath != null ? defaultPath : "Y:")+"/el267.msh");
        ...
        System.out.println("Source(" + f + ")= " + ib[0][ib[1].length - 2] + " + j* " + ib[0][ib[1].length - 1]);
        ...
        ExportGmshHdiv exportJreal = new ExportGmshHdiv(Omega, (defaultPath != null ? defaultPath : "D:") + "/jsiau/_Backup_Sources/Resultats/Dielectric/SNAKE/S_RE_f" + f + "_nDof" + Omega.getActiveDofCount() + ".msh");
        exportJreal.addQuantity(Jreal, "Jreal");

        ExportGmshHdiv exportJimag = new ExportGmshHdiv(Omega, (defaultPath != null ? defaultPath : "D:") + "/jsiau/_Backup_Sources/Resultats/Dielectric/SNAKE/S_IM_f" + f + "_nDof" + Omega.getActiveDofCount() + ".msh");
        exportJimag.addQuantity(Jimag, "Jimag");

        ComplexFaceQuantity J = new ComplexFaceQuantity(Omega, res);
        exportJreal = new ExportGmshHdiv(Omega, (defaultPath != null ? defaultPath : "D:") + "/jsiau/_Backup_Sources/Resultats/Dielectric/SNAKE/S_Jmod" + f + "_nDof" + Omega.getActiveDofCount() + ".msh");
