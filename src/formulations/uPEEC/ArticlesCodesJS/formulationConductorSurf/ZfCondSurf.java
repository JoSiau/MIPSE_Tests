...
public class ZfCondSurf {
    ...
    public static void main(String[] args) {
        ...
        int[] ind = new int[]{2, -1, 1, -1, -3};
        int cct[] = null;
        meshDir += "/magnetodynamic/U_PEEC_DIELECTRIC/ArticlesCodesJS/SERPSYM_3.DEC";
        ImportFlux mesh = new ImportFlux(meshDir);
        SurfaceRegion conductor = (SurfaceRegion) mesh.getRegion(ind[0]);
        LineRegion bordNull;
        if (ind[1] >= 0) {
            bordNull = (LineRegion) mesh.getRegion(ind[1]);
        } else {
            bordNull = (LineRegion) conductor.generateBorder();
        }
...
//        solP.setPtsDeGaussInductifsSurf(25, 16, 16);// Quandrangle
        ...
        if (meshDir.endsWith("SERPSYM_5.DEC")) {
            circuitPur.addSourceISimple(nbBranches, 3647, 3646, "Source I", 1.0, 0.0);

        } else {
            circuitPur.addSourceISimple(nbBranches, 1479, 1478, "Source I", 1.0, 0.0);
            cct = new int[]{874, 875};
        }
...
//        solP.setExportTopologie("D:");
        ...
        double ftab[] = new double[10];
//        ftab = compteFrequenciesLog(6, 7, ftab.length);
        ftab = ZfSerpentSym.getFrequencies();
        ...
        for (int iF = 0; iF < ftab.length; iF++) {
            ...
            /*/ // Scale tous les dof
             res.scale(1/ep);
             //*/
            imp[iF] = Math.hypot(ib[1][2 * nbBranches], ib[1][2 * nbBranches + 1]) / Math.hypot(ib[0][2 * nbBranches], ib[0][2 * nbBranches + 1]);
            //
            //
            ...
            System.out.println("Impedances computed : ");
            for (int i = 0; i <= iF; i++) {
