...
import g2elab.mipse.mipseCore.blockAssembly.real.RealMatrixBlocAdd;
...
import g2elab.mipse.mipseCore.numericalMethods.*;
import g2elab.mipse.mipseCore.storage.StorageBasic2D;
...
import g2elab.mipse.numericalTools.matrix.real.bloc.RealMatrixAssembly;
import g2elab.mipse.numericalTools.matrix.real.dense.basic2D.Basic2D;
import g2elab.mipse.numericalTools.matrix.real.dense.basic2D.SolverLUBasic2D;
 ...
 * @author jsiau
 */
public class CEFC_FullPaper implements ProductReal {
    ...
    final boolean recomp = true;
    final boolean agglo = recomp;

    double epsSolver = 1e-6;
    //
    // STORAGES
    //
    boolean isHmatrix = true;
    private SparseMatrixRowReal SFE;
    private StorageHmatrix precond;
    ...
    RealMatrixAssembly prod;

    public static void main(String[] args) {
        CEFC_FullPaper cefc_FullPaper = new CEFC_FullPaper();
    }
    ...
    private final String pathOut = "D:/jsiau/_Backup_Sources/Resultats/CEFC14/";

    public CEFC_FullPaper() {
...
//            4098 12938 26420 48154 79380
                switch (scSize) {
                    case 0:
                        pathMesh = "D:/jsiau/_Backup_Sources/Meshs/Sphere/SPHERE_" + "4098.DEC";
                        nDof = 4098;
                        break;
                    case 1:
                        pathMesh = "D:/jsiau/_Backup_Sources/Meshs/Sphere/SPHERE_" + "11426.DEC";
                        nDof = 11426;
                        break;
                    case 2:
                        pathMesh = "D:/jsiau/_Backup_Sources/Meshs/Sphere/SPHERE_" + "45156.DEC";
                        nDof = 45156;
                        break;
                    default:
                ...
                System.out.println("Selectionner la taille du maillage:\n0= 3846\n1= 12938\n2= 48154\n3= 79380");
                int scSize = sc.nextInt();
//            pathSC = "D:/jsiau/_Backup_Sources/Meshs/SphereCreuse/SC_" + "3846.DEC";// 3846 12938 26420 48154 79380
                switch (scSize) {
                    case 0:
                        pathMesh = "D:/jsiau/_Backup_Sources/Meshs/SphereCreuse/SC_" + "3846.DEC";
                        nDof = 3846;
                        break;
                    case 1:
                        pathMesh = "D:/jsiau/_Backup_Sources/Meshs/SphereCreuse/SC_" + "12938.DEC";
                        nDof = 12938;
                        break;
                    case 2:
                        pathMesh = "D:/jsiau/_Backup_Sources/Meshs/SphereCreuse/SC_" + "48154.DEC";
                        nDof = 48154;
                        break;
                    case 3:
                        pathMesh = "D:/jsiau/_Backup_Sources/Meshs/SphereCreuse/SC_" + "79380.DEC";
                        nDof = 79380;
                        break;
                    default:
                        throw new InternalError("Miss tip ?");
            ...
            nDof = 29362;
            pathMesh = "D:/jsiau/_Backup_Sources/Meshs/CEFC14/Problem13/P13_30KP.DEC";
...
//             }
            ...
            try {
                ...
                Logger.getLogger(CEFC_FullPaper.class.getName()).log(Level.SEVERE, null, ex);
            }
            return res;
        } else {
    ...
    protected double[] getRandomVector() {
        ...
        try {
            ...
            try {
                ...
                Logger.getLogger(CEFC_FullPaper.class.getName()).log(Level.SEVERE, null, ex1);
            }
        }
        return resFull;
    }
...
//                H.scale(Qi);
            ...
            /*
            FiniteElementFormulationHmatrix hfe = new FiniteElementFormulationHmatrix(Target, nmin, eta);
            hfe.assembly(nbGauss);
            H.printOnJFrame("H");
            this.precond = (StorageHmatrix) hfe.getStore();
            precond.printOnJFrame("FE");
            precond.mulAddH(Qi, H, new TruncationControl("rel", eps));
            if (agglo) {
                H.Agglomerate(new TruncationControl("rel", eps));
            }
            H.printOnJFrame("H2");

            precond.printOnJFrame("H+FE");
            double epsPrec = 3e-1;
            precond.Agglomerate(new TruncationControl("rel", epsPrec));
            precond.printOnJFrame("H+FE agglo");
            HmatrixLUDecomposition hlu = new HmatrixLUDecomposition(precond, new TruncationControl("rel", epsPrec));
            FGMResReal solver = new FGMResReal(this, hlu);
            /*/
            if (agglo) {
                H.Agglomerate(new TruncationControl("rel", eps));
            }
            H.printOnJFrame("H2");
             ...
             //*/
            solver.setInfoResolution(new double[]{1000, -1e-8, 1, -50});
            double[] x = new double[nDof];
            double[] secondMembre = getSecondMembre(Target, nDof);
            System.out.println("secondMembre= " + Arrays.toString(secondMembre));
            deb = System.nanoTime();
            x = solver.solve(x, secondMembre);
            fin = System.nanoTime();
...
//        computeEF(Target, nbGauss);
        ...
        GalerkinIntegralFormulationFMM f = new GalerkinIntegralFormulationFMM(Target, Source, Kernel, strategy, nbGauss, nbGauss,
                new RepartitionElemNbMaxElem(nmin), 1.0, 3, 1, 1);
        deb = System.nanoTime();
        f.assembly();
    ...
    public double[] product(double[] x, double[] res) {
        if (prod == null) {
            prod = new RealMatrixAssembly(1, 1);
            // SFE + Qi * H
            prod.setBloc(0, 0, new RealMatrixBlocAdd(SFE, new RealMatrixBlocScale(Qi, H)));
        }
        if (!electroStatic && isHmatrix) {
            res = prod.product(x, res);
            return res;
        }
        if (res == null) {
            res = new double[nDof];
